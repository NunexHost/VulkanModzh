...
//    UniformBuffers uniformBuffers;
    ...
    private WorldRenderer(RenderBuffers renderBuffers) {
        ...
        Renderer.getInstance().addOnResizeCallback(() -> {
            if(this.indirectBuffers.length != Vulkan.getFrameCount())
                allocateIndirectBuffers();
        });
    }
    ...
    private void allocateIndirectBuffers() {
        ...
        this.indirectBuffers = new IndirectBuffer[Vulkan.getFrameCount()];
...
//        p.pushMilestone("layer " + layerName);
        ...
        GraphicsPipeline pipeline = TerrainShaderManager.getTerrainShader(renderType);
